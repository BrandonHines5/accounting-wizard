...
the package (and the test suite) runs without it. For large weekly volumes the
synchronous judge can be swapped for `AnthropicBatchJudge` (Message Batches API,
50% cost) behind the same `Judge` interface — `--tier3 batch` in skill.run.
"""
